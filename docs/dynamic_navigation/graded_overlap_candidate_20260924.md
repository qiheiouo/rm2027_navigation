...
首次启动记录 `rank_phase4_20260924_01` 在导航前退出 2：观察器要求项目约定的隔离 `ROS_DOMAIN_ID=174`，脚本误用了 175。插件已加载，但未形成控制周期或动作结果。原失败目录保留；修正 domain 后的新 series 是首次实际导航试次，不把启动失败当作算法样本。

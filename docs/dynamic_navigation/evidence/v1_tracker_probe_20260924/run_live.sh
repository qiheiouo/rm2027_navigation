#!/usr/bin/env bash
# Container entry point for one fixed-phase, shadow-only tracker probe.
set -eo pipefail
...
source /work/dynamic_prediction_v1_build/install/setup.bash
...
setsid python3 /ws/docs/dynamic_navigation/evidence/v1_tracker_probe_20260924/live_nodes.py map \
...
setsid ros2 run rm_dynamic_obstacle_tracking dynamic_obstacle_tracker_node \
  ...
  -p use_sim_time:=true -p map_topic:=/prediction_v1/static_map -p scan_topic:=/scan \
  -p prediction.velocity_decay_tau:=0.0 > "$out/tracker.log" 2>&1 & pids+=($!)
set +e

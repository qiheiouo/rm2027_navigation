...
SERIES=WORK/'runs/dynamic_prediction_mppi_multiphase_v2'
PHASES=(0,2,4,6)
...
def run(planner,mode,phase):
    ...
    paths=[HERE/n for n in ('run_multiphase_v2.py','run_trial.sh','static_map.py','audit.py')]
    ...
    try:
        ...
        if 'nonplanar pose requires 3D projection audit' not in str(error):
            raise
        result={'planner':planner,'mode':mode,'evidence_valid':False,
          'limited_dynamic_gate_pass':False,'audit_error':str(error),
          'scope':'Existing independent planar oracle rejected nonplanar Gazebo pose; no geometry pass claimed.'}
    result['phase_seconds']=phase
    with (trial/'ab_audit.json').open('x') as f:json.dump(result,f,indent=2,allow_nan=False);f.write('\n')
    print(json.dumps({'planner':planner,'mode':mode,'phase':phase,'action':result.get('action_status'),
      'recoveries':result.get('recoveries'),'body_bound':result.get('geometry',{}).get('body',{}).get('moving_interpolation_bound_m'),
      'padded_bound':result.get('geometry',{}).get('padded',{}).get('moving_interpolation_bound_m'),

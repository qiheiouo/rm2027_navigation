...
历史未来真值拟合版本在这个周期保留了轨迹区分度，但其经验包络在旧试次留一验证中已经有漏包；过去扫描与条件加速度界版本覆盖了已检查真值，却让预测后段的硬碰撞项重新饱和。两者共同说明目前的瓶颈是**可观测输入、运动约束和可用预测占用之间的取舍**，不能靠增加 V1 weight 解决。下一项研发应先验证可部署传感器下的源中心误差与障碍物运动能力，再对明确的冻结周期量化“覆盖与可行 rollout 数”的边界；在此之前不替换运行 critic，也不把离线零漏包称作稳定安全。

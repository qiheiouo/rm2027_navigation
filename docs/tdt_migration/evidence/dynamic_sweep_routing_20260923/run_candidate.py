#!/usr/bin/env python3
"""Run one prespecified QP fixture-sweep planning pilot in a fresh series."""
import hashlib
import json
import os
import subprocess
...
from planner_sweep import patch_profile, sweep_points

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[3]
WORK = ROOT / "build/tdt_p2b"
RUNS = WORK / "runs/dynamic_sweep_routing_pilot_v1"
...
def prepare():
    ...
    old = WORK / "runs/dynamic_guard_pilot_v1/profiles/tdt_qp.yaml"
    old_profile = yaml.safe_load(old.read_text())
    changed = patch_profile(old_profile)
    assert changed["controller_server"] == old_profile["controller_server"]
    profile = RUNS / "profile.yaml"
    profile.write_text(yaml.safe_dump(changed, sort_keys=False))
    assert yaml.safe_load(profile.read_text()) == changed
    ...
    write_new(RUNS / "inputs.json", {
        "schema": "tdt_dynamic_sweep_routing_inputs/v1",
        ...
        "semantic_profile_difference": "Only the same SDF-derived marking-only fixture_sweep_layer added to both costmaps; no other parsed YAML change.",
...
def verify():
    ...
    old = yaml.safe_load((WORK / "runs/dynamic_guard_pilot_v1/profiles/tdt_qp.yaml").read_text())
    assert yaml.safe_load((RUNS / "profile.yaml").read_text()) == patch_profile(old)
...
def run():
    ...
    for env in ("ROS_DOMAIN_ID=174", "ROS_LOCALHOST_ONLY=1",
                ...
                "TDT_PHASE_SECONDS=0", "IGN_PARTITION=tdt_dynamic_sweep_routing_v1"):
        args += ["-e", env]
    args += ["rm2027_navigation:humble",
             "/ws/docs/tdt_migration/evidence/dynamic_sweep_routing_20260923/run_trial.sh",

#!/usr/bin/env python3
"""Run one fresh QP pilot with next-decision stop viability."""
import hashlib
import json
import os
import subprocess
...
RUNS = WORK / "runs/dynamic_viability_guard_pilot_v1"
...
def prepare():
    ...
    old = WORK / "runs/dynamic_guard_horizon_pilot_v2/profile.yaml"
    ...
    write_new(RUNS / "inputs.json", {
        "schema": "tdt_dynamic_viability_guard_inputs/v1",
        ...
        "previous_series_inputs_sha256": sha(WORK / "runs/dynamic_guard_horizon_pilot_v2/inputs.json"),
        ...
        "semantic_profile_difference": "None versus horizon guard v2; profile is byte-identical. Command admission additionally requires next-decision stop viability.",
        "files": {str(p.relative_to(ROOT)): sha(p) for p in script_paths},
        "scope": "One fixed-phase viability-guard simulation diagnostic; no deployment/hardware acceptance."
    })
...
def run():
    ...
    write_new(trial / "metadata.json", {
        "planner": "tdt_qp", "trial": 1, "mode": "viability_guard_and_planner_sweep",
    ...
    for env in ("ROS_DOMAIN_ID=174", "ROS_LOCALHOST_ONLY=1",
                ...
                "TDT_PHASE_SECONDS=0", "IGN_PARTITION=tdt_dynamic_viability_guard_v1"):
        args += ["-e", env]
    args += ["rm2027_navigation:humble",
             "/ws/docs/tdt_migration/evidence/dynamic_viability_guard_20260924/run_trial.sh",

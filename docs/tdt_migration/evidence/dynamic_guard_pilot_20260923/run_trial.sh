#!/usr/bin/env bash
# Isolated container entry point; fixed fixture and profile, optional final filter.
set -eo pipefail
trial_dir=$1
profile=$2
mode=$3
case "$trial_dir" in /work/runs/*) ;; *) exit 2;; esac
case "$mode" in baseline|guard) ;; *) exit 2;; esac
...
trap 'exit 130' INT
trap 'exit 143' TERM
launch=/ws/docs/tdt_migration/evidence/dynamic_reference_20260922/dynamic.launch.py
if [[ "$mode" == guard ]]; then
  launch=/ws/docs/tdt_migration/evidence/dynamic_guard_pilot_20260923/dynamic_guard.launch.py
  setsid python3 /ws/docs/tdt_migration/evidence/dynamic_guard_pilot_20260923/guard_node.py \
    "$profile" "$trial_dir/guard_decisions.jsonl" > "$trial_dir/guard.log" 2>&1 &
  guard_pid=$!
fi
setsid ros2 launch "$launch" params_file:="$profile" > "$trial_dir/launch.log" 2>&1 &
launch_pid=$!

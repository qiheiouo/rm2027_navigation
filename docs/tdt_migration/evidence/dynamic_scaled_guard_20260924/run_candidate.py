#!/usr/bin/env python3
"""Run one fresh QP pilot with same-direction safe command scaling."""
import hashlib
import json
import os
import subprocess
...
RUNS = WORK / "runs/dynamic_scaled_guard_pilot_v1"
...
def prepare():
    ...
    old = WORK / "runs/dynamic_sweep_routing_pilot_v2/profile.yaml"
    ...
    write_new(RUNS / "inputs.json", {
        "schema": "tdt_dynamic_scaled_guard_inputs/v1",
        ...
        "previous_series_inputs_sha256": sha(WORK / "runs/dynamic_sweep_routing_pilot_v2/inputs.json"),
        ...
        "semantic_profile_difference": "None versus sweep-routing v2; profile is byte-identical. Only final-command rejection becomes same-direction certified speed scaling.",
        "files": {str(p.relative_to(ROOT)): sha(p) for p in script_paths},
        "scope": "One fixed-phase scaled-guard simulation diagnostic; no deployment/hardware acceptance."
    })
...
def run():
    ...
    write_new(trial / "metadata.json", {
        "planner": "tdt_qp", "trial": 1, "mode": "scaled_guard_and_planner_sweep",
    ...
    for env in ("ROS_DOMAIN_ID=174", "ROS_LOCALHOST_ONLY=1",
                ...
                "TDT_PHASE_SECONDS=0", "IGN_PARTITION=tdt_dynamic_scaled_guard_v1"):
        args += ["-e", env]
    args += ["rm2027_navigation:humble",
             "/ws/docs/tdt_migration/evidence/dynamic_scaled_guard_20260924/run_trial.sh",

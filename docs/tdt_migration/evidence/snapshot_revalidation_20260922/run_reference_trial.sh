...
trap 'exit 130' INT
trap 'exit 143' TERM
setsid ros2 launch /ws/docs/tdt_migration/evidence/snapshot_revalidation_20260922/reference.launch.py \
  params_file:="$profile" > "$trial_dir/launch.log" 2>&1 &
launch_pid=$!
observer_extra=()
if [[ "${TDT_HEADING_AB:-0}" == 1 ]]; then observer_extra=(--verify-profile "$profile"); fi
set +e
python3 /ws/docs/tdt_migration/evidence/snapshot_revalidation_20260922/observe_reference.py \

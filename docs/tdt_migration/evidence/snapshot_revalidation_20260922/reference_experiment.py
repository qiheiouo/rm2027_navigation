...
ROOT=WORK/'runs/snapshot_revalidation_v1'
...
def prepare():
    ...
    write_new(ROOT/'manifest.json',{'schema':'rm_tdt_planner/new_car_geometry_reference/v1',
        ...
        'diagnostic_change':'Approved latest-snapshot admission; planner DEBUG and observer unchanged from reference v2.',
...
def analyze_trial(path):
    ...
    report={'planner':original['planner'],'trial':original['trial'],'reference_geometry':'existing_new_car_126mm',
        ...
        'snapshot_admissions':[{'sim_t':e['t'],**json.loads(e['message'].split('snapshot_admission_v1=',1)[1])} for e in diagnostics if e['message'].startswith('snapshot_admission_v1=')],

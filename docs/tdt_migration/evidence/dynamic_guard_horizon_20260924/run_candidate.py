#!/usr/bin/env python3
"""Run one fresh QP pilot with explicit next-cycle stop reserve."""
import hashlib
import json
import os
import subprocess
...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "dynamic_sweep_routing_v2_20260923"))
from planner_sweep_v2 import sweep_points

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[3]
WORK = ROOT / "build/tdt_p2b"
RUNS = WORK / "runs/dynamic_guard_horizon_pilot_v1"
...
def prepare():
    ...
    old = WORK / "runs/dynamic_scaled_guard_pilot_v1/profile.yaml"
    ...
    script_paths += [HERE.parent / "dynamic_sweep_routing_v2_20260923" / n
                     for n in ("sweep_cloud.py", "planner_sweep_v2.py")]
    ...
    write_new(RUNS / "inputs.json", {
        "schema": "tdt_dynamic_guard_horizon_inputs/v1",
        ...
        "previous_series_inputs_sha256": sha(WORK / "runs/dynamic_scaled_guard_pilot_v1/inputs.json"),
        ...
        "semantic_profile_difference": "None versus scaled guard v1; profile is byte-identical. The command certificate reserves an additional control interval and measured odom age.",
        "files": {str(p.relative_to(ROOT)): sha(p) for p in script_paths},
        "scope": "One fixed-phase horizon-guard simulation diagnostic; no deployment/hardware acceptance."
    })
...
def run():
    ...
    write_new(trial / "metadata.json", {
        "planner": "tdt_qp", "trial": 1, "mode": "horizon_guard_and_planner_sweep",
    ...
    for env in ("ROS_DOMAIN_ID=175", "ROS_LOCALHOST_ONLY=1",
                "PYTHONDONTWRITEBYTECODE=1", "TMPDIR=/work/tmp",
                "LIBGL_ALWAYS_SOFTWARE=true", "QT_QPA_PLATFORM=offscreen",
                "TDT_PHASE_SECONDS=0", "IGN_PARTITION=tdt_dynamic_guard_horizon_v1"):
        args += ["-e", env]
    args += ["rm2027_navigation:humble",
             "/ws/docs/tdt_migration/evidence/dynamic_guard_horizon_20260924/run_trial.sh",

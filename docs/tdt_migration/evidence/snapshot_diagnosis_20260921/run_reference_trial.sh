...
trap 'exit 130' INT
trap 'exit 143' TERM
setsid ros2 launch /ws/docs/tdt_migration/evidence/snapshot_diagnosis_20260921/reference.launch.py \
  params_file:="$profile" > "$trial_dir/launch.log" 2>&1 &
launch_pid=$!
observer_extra=()
if [[ "${TDT_HEADING_AB:-0}" == 1 ]]; then observer_extra=(--verify-profile "$profile"); fi
set +e
python3 /ws/docs/tdt_migration/evidence/snapshot_diagnosis_20260921/observe_reference.py \
...
set -e
python3 /ws/docs/tdt_migration/evidence/snapshot_diagnosis_20260921/capture_runtime.py "$profile" "$trial_dir/runtime_geometry.json" > "$trial_dir/geometry_capture.log" 2>&1

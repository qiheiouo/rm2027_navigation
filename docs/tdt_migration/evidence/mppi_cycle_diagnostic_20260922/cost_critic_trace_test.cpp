...
TEST(DiagnosticCostCritic, RecordsFreeAndFullyBlockedCycles)
{
  ...
  params.emplace_back("dummy.critics", std::vector<std::string>{"CostCritic", "GoalCritic"});
  params.emplace_back("dummy.CostCritic.consider_footprint", true);
  ...
  ASSERT_TRUE(costmap_ros->set_parameter(rclcpp::Parameter("footprint", "[[0.01,0.01],[-0.01,0.01],[-0.01,-0.01],[0.01,-0.01]]")).successful);
  costmap_ros->on_configure(rclcpp_lifecycle::State{});
  *(costmap_ros->getCostmap()) = *getDummyCostmap(costmap_settings);
  ...
  auto raw = costmap_ros->getCostmap();
  std::fill_n(raw->getCharMap(), raw->getSizeInCellsX() * raw->getSizeInCellsY(), 254);
  EXPECT_THROW(controller->computeVelocityCommands(pose, velocity, {}), std::runtime_error);

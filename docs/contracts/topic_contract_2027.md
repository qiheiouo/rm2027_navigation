...
| `/cmd_vel` | `geometry_msgs/msg/Twist` | Nav2 | normal serial path or one explicitly enabled final velocity gate | Commanded chassis velocity in `base_link`; never connect both the raw and gated path to real serial in one run |
| `/cmd_vel_dog_hole_gated` | `geometry_msgs/msg/Twist` | opt-in `dog_hole_entry_pause_gate` | old-car serial transport only while that gate is active | Unchanged Nav2 command outside the entry pause; forced zero during braking/hold; map-bound and default-off |
| `/dog_hole/pause_state` | `std_msgs/msg/String` | opt-in `dog_hole_entry_pause_gate` | diagnostics, rosbag | Observable `waiting_for_path/armed/braking/holding/released/committed/passed/invalid_entry` state; not an authority input |
| `/dog_hole/pause_active` | `std_msgs/msg/Bool` | opt-in `dog_hole_entry_pause_gate` | diagnostics, rosbag | True only during braking or the timed hold |
| `/dog_hole/path_crosses` | `std_msgs/msg/Bool` | opt-in `dog_hole_entry_pause_gate` | diagnostics, rosbag | Whether the currently accepted standard Nav2 path intersects a bound committed corridor |

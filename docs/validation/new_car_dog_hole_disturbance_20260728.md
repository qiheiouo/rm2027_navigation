...
> 2026-08-04 更正：本文“模型受平面约束”的判断已被后续直接证据推翻。
> 根因是 `ros_gz_sim create` 把生成场景的配置 pose 覆盖为世界原点，机器人
> 实际没有经过位于错误位置的坡面。修复实体 pose 后已测得真实
> `z≈0.07 m、pitch≈9.16°`；几何结论以
> `new_car_dog_hole_3d_geometry_20260804.md` 为准。本文二维扰动数据仍保留为
> 当时的软件控制证据，但不能作为物理墙体、坡面或洞顶证据。

...
public:
    ...
    tf_listener_(tf_buffer_)
  {
    ...
    subscription_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&AutomaticRampFilterNode::cloud_callback, this, std::placeholders::_1));

    RCLCPP_WARN(
      get_logger(),
      "automatic ramp filter is %s: %s -> %s, detection_frame=%s confirm=%zu",
      shadow_only_ ? "SHADOW-ONLY" : "ACTIVE", input_topic_.c_str(),
      output_topic_.c_str(), detection_frame_.c_str(), confirmation_frames_);
  }
...
private:
  ...
  void validate_parameters()
  {
    ...
    if (transform_timeout_sec_ < 0.0 || min_forward_ >= max_forward_ ||
      ...
      max_slope_change_rad_ <= 0.0 || max_yaw_change_rad_ <= 0.0)
    {
  ...
  void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr message)
  {
    ...
    if (!filter_enabled_) {
      ...
      publish_diagnostic(
        ...
        "fail-closed passthrough: disabled", input_count, 0U, 0U, std::nullopt);
      return;
    }
    ...
    std::vector<rm_mid360_driver_bridge::Point3d> detection_points;
    detection_points.reserve(voxels.size());
    for (const auto & item : voxels) {
      detection_points.push_back(item.second.point);
    }
    const auto detection = rm_mid360_driver_bridge::detect_automatic_ramp(
      detection_points, detection_config_);
    update_track(detection);

    if (!confirmed_detection_) {
      publisher_->publish(*message);
      publish_diagnostic(
        diagnostic_msgs::msg::DiagnosticStatus::WARN,
        detection ? "candidate accumulating; fail-closed passthrough" :
        "no ramp candidate; fail-closed passthrough",
        input_count, detection_points.size(), 0U, detection);
      return;
    }
    ...
    publish_diagnostic(
      ...
      input_count, detection_points.size(), removed_count, detection);
  }
  ...
  void publish_diagnostic(
    ...
    std::size_t detection_point_count,
    ...
    const std::optional<rm_mid360_driver_bridge::AutomaticRampDetection> & detection)
  {
    ...
    status.values = {
      ...
      key_value("candidate_present", detection ? "true" : "false"),
      ...
      key_value("input_points", std::to_string(input_count)),
      key_value("detection_points", std::to_string(detection_point_count)),
      key_value("removed_surface_points", std::to_string(removed_count)),
    };
    if (detection) {
      status.values.push_back(
        key_value(
          "slope_deg", std::to_string(detection->region.slope_rad * 180.0 / kPi)));
      status.values.push_back(key_value("length", std::to_string(detection->length)));
      status.values.push_back(key_value("width", std::to_string(detection->width)));
      status.values.push_back(
        key_value(
          "inlier_ratio", std::to_string(detection->inlier_ratio)));
      status.values.push_back(
        key_value(
          "rms_residual", std::to_string(detection->rms_residual)));
    }
